...
def compound(s):
    if len(s) < 1:
        ...
        return [[add_prefix(a,pre1)] + c for (pre1,suf1) in saldo_util.inits(s) if len(pre1) > 1 for (pre,suf) in sandhi(pre1,suf1) for a in saldo_util.lookup_ff(pre) if  suf != [] or a['msd'] in ['ci','cm','c'] for c in compound(suf)]
...
def ok_compound(c):
    if len(c) == 1:
        return not (c[0]['msd'] in ['ci','cm','c']) and not_fragment(c[0]['msd']) and (c[0]['pos'][-1] != 'h')
    else:
        if not(c[-1]['msd'] in ['ci','cm','c']) and not_fragment(c[0]['msd']) and (c[-1]['pos'] in ['nn','vb','av','ab','pm'] or c[-1]['pos'][-1] == 'h') and c[0]['msd'] in ['ci','c']:
