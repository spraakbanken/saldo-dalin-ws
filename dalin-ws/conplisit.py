...
def retrieve_data(search_ref, start, end):
    get_result_address = (
        lb
        + "?action=get-result-set&searchref=%s&username=sblex&resultitem=%d&resultlength=%s"
        % (search_ref, start, end - start + 1)
    )
        ...
        result.append(
            {
                ...
                "name": title,
                "url": "http://litteraturbanken.se/#forfattare/%s/titlar/%s/sida/%s/etext?traff=%s"
                % (authorid, title_id, page_name, nodeid),
            }
        )
    return result
...
def table(sense, hits=50):
    ...
    for hit in req.retrieve(end=number_of_hits):
        if len(hit["name"]) > 25:
            ...
            name = hit["name"]
        result += (
            '<tr><td>%d</td><td>%s</td><td><a href="%s">%s</a></td><td>%s</td><td>%s</td></tr>'
            % (count, hit["left"], hit["url"], hit["kw"], hit["right"], name)
        )
        count += 1
    result += "</table>"
    return result
