...
def lid_ref(word,lid):
    if lid=='':
        ...
        return ('<a href="http://litteraturbanken.se/query/dalin.xql?word=%s&limit=10">'%word) + lid +'</a>'
...
def korpus_ref(lids,name):
    xs  = [y for lid in lids for x in generate_wordforms(utf8.e(lid)) for y in x.split(' ')] #-- ' '.join( #).encode('UTF-8')
    ys = []
    for x in xs:
        if not (x in ys):
            ys.append(x)
