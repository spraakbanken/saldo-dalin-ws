...
def htmlize(paradigm,word,j):
    if (j == []):
            content = '<center><p><b>paradigm ' + utf8.d(paradigm) + ' finns ej.</b></p></center>'
            return saldo_util.html_document(utf8.d(paradigm) + ' "' + utf8.d(word) + '"',content)
    content='<center><table border="1"><tr><td><b>%s</b></td><td>%s</td></tr>' % ("word class",j[0]['pos'])
    if(len(j[0]['inhs']) > 0):
        content+='<tr><td><b>%s</b></td><td>%s</td></tr>' % ("inherent feature(s)",", ".join(j[0]['inhs']))
