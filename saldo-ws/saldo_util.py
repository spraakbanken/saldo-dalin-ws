...
def lsib(lemma_id):
    ...
    return [
        l
        for s in senses
        for _s in sib(utf8.e(s))
        for l in lookup_lid(utf8.e(_s))["l"]
        if utf8.d(lemma_id) != l
    ]
...
def gen_ref(p, w, l):
    return '<a href="http://spraakbanken.gu.se/ws/saldo-ws/gen/html/%s/%s">%s</a>' % (
...
def sms_ref(w, l):
    return '<a href="http://spraakbanken.gu.se/ws/saldo-ws/sms/html/%s">%s</a>' % (
        urllib.parse.quote(w),
        l,
    )
